...
void CTFFlagSetup (edict_t *ent)
{
	...
	if (tr.startsolid)
	{
		gi.dprintf(DEVELOPER_MSG_GAME, "CTFFlagSetup: %s startsolid at %s\n", ent->classname, vtos(ent->s.origin));
		G_FreeEdict (ent);
		return;
	}
...
//PON-CTF
	if(chedit->value && ent == &g_edicts[1] && ent->client->ctf_grapple)
	{
		...
		if(Route[CurrentIndex - i].state == GRS_GRAPHOOK)
		{
			...
			Route[CurrentIndex].Vectors.Tcourner[0] = VectorLength(v);
...
//PON-CTF
	if(chedit->value && self->owner == &g_edicts[1])
	{
		...
		while(--i > 0)
		{
			...
			{
				VectorCopy(self->s.origin,Route[i].Vectors.Tcourner);
				break;
			}
		}
...
//PONKO
	...
	if(fpout == NULL)
	{
		if(!ctf->value) gi.dprintf(DEVELOPER_MSG_GAME, "Chaining: file %s.chn not found.\n",level.mapname);
		else gi.dprintf(DEVELOPER_MSG_GAME, "Chaining: file %s.chf not found.\n",level.mapname);
	}
	else
	{
		...
		{
			CurrentIndex = 0;
			gi.dprintf(DEVELOPER_MSG_GAME, "Chaining: %s.chn is not a chaining file.\n",level.mapname);
			fclose(fpout);
			return;
		}
		gi.dprintf(DEVELOPER_MSG_GAME, "Chaining: %s.chn founded.\n",level.mapname);
...
gi.dprintf(DEVELOPER_MSG_GAME, "GRS_TELEPORT\n");
					...
					if (other->inuse)
					{
							...
							|| Route[i].state == GRS_ONDOOR)
						{
							...
							if(VectorCompare (Route[i].Pt,v/*other->monsterinfo.last_sighting*/))
							{
								...
								{
//gi.dprintf(DEVELOPER_MSG_GAME, "assingned %s\n",other->classname);
									Route[i].ent = other;
									break;
								}
								//train
...
//gi.dprintf(DEVELOPER_MSG_GAME, "assingned %s\n",other->classname);
							...
							|| Route[i].state == GRS_REDFLAG || Route[i].state == GRS_BLUEFLAG)
						{
//else gi.dprintf(DEVELOPER_MSG_GAME, "CYAU!!!!!!!\n");
...
//gi.dprintf(DEVELOPER_MSG_GAME, "HATA ATTA!!!!!!!\n");
//gi.dprintf(DEVELOPER_MSG_GAME, "assingned %s\n",other->classname);
									Route[i].ent = other;
									break;
								}
							...
							{
//if(Route[i].state == GRS_REDFLAG || Route[i].state == GRS_BLUEFLAG)
//gi.dprintf(DEVELOPER_MSG_GAME, "HATA Dame!!!!!!!\n");
							}
						}
					}
				}
				if(j >= globals.num_edicts && (Route[i].state == GRS_ITEMS || Route[i].state == GRS_REDFLAG || Route[i].state == GRS_BLUEFLAG)) gi.dprintf(DEVELOPER_MSG_GAME, "kicked item\n");
				if(j >= globals.num_edicts) Route[i].state = GRS_NORMAL;
			}
		}
		gi.dprintf(DEVELOPER_MSG_GAME, "Chaining: Total %i chaining pod assigned.\n",CurrentIndex);
		fclose(fpout);
	}
	return;
}
