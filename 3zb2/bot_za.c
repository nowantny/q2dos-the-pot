...
void Get_RouteOrigin(int index,vec3_t pos)
{
	...
	else if(Route[index].state == GRS_ONTRAIN)
	{
		...
		{
			if(VectorCompare(Route[index].Vectors.Tcourner,Route[index].ent->target_ent->s.origin))
			{
				VectorCopy(Route[index].ent->union_ent->s.origin,pos);
				pos[2] += 8;
				return;
		...
		while(1)
		{
			...
			if(e->target_ent)
			{
				if(VectorCompare(Route[index].Vectors.Tcourner,e->target_ent->s.origin))
				{
...
//						VectorCopy(Route[zc->routeindex - 1].Pt,ent->s.origin);
						VectorSubtract(Route[zc->routeindex - 1].Vectors.Tcourner,vv,v);
				...
				&& ent->client->ctf_grapple) 
			{
				...
				else if(ent->client->ctf_grapplestate == CTF_GRAPPLE_STATE_PULL)
				{
					if(1)
					{
						...
						yaw = VectorLength(v);
						if(yaw <= (Route[zc->routeindex].Vectors.Tcourner[0] /*+ 32*/))
						{
...
//					gi.bprintf(PRINT_HIGH,"length %f < %f\n",Route[zc->routeindex].Vectors.Tcourner[0],VectorLength(v));
				}
				else if(ent->client->ctf_grapplestate == CTF_GRAPPLE_STATE_HANG)
				{
/*					if((zc->routeindex + 1) < CurrentIndex)
					{
						...
						{
							if(TraceX(ent,Route[zc->routeindex + 1].Vectors.Tcourner)) k = true; 
						}
						else k = true;
					}*/
...
//gi.bprintf(PRINT_HIGH,"Groff 0!\n");
				...
				if(k)
				{
					if(zc->routeindex < CurrentIndex)
					{
						if(Route[zc->routeindex].state == GRS_GRAPSHOT)
						{
							if(1/*TraceX(ent,Route[zc->routeindex + 1].Vectors.Tcourner)*/) zc->routeindex++;
						}
					}
				}
			}
...
//			gi.bprintf(PRINT_HIGH,"I am carrierX!!\n");
		...
		{
			if(Route[zc->routeindex].state < GRS_ITEMS
				&& Route[zc->routeindex].Vectors.linkpod[MAXLINKPOD - 1])
			{
				k = Route[zc->routeindex].Vectors.linkpod[MAXLINKPOD - 1];
				if(j == FOR_FLAG1)
				{
					if(k & CTF_FLAG2_FLAG) 
					{
//gi.bprintf(PRINT_HIGH,"Wrong way 1\n");
						...
						{
							if(!Route[zc->routeindex].Vectors.linkpod[i]) break;
							k = Route[Route[zc->routeindex].Vectors.linkpod[i]].Vectors.linkpod[MAXLINKPOD - 1];
							if(!(k & CTF_FLAG2_FLAG))
							{
								zc->routeindex = Route[zc->routeindex].Vectors.linkpod[i];// zc->route_trace = false;
								zc->havetarget = false;
//gi.bprintf(PRINT_HIGH,"fixed for flag 1\n");
					...
					{
						zc->havetarget = true;
						zc->targetindex = Route[zc->routeindex].Vectors.linkpod[MAXLINKPOD - 1] & 0x7FFF;
					}
				}
				else if(j == FOR_FLAG2)
				{
...
//アイテムリンクチェック1>>
								if(Route[zc->routeindex].state <= GRS_ITEMS)
								{
									if(zc->havetarget)
									{
										...
										{
											k = Route[zc->routeindex].Vectors.linkpod[i];
											if(!k) break;
...
//アイテムリンクチェック2>>
											if(Route[zc->routeindex].state <= GRS_ITEMS)
											{
												if(zc->havetarget)
												{
													for(i = 0;i < (MAXLINKPOD - (ctf->value != 0));i++)
													{
														j = Route[zc->routeindex].Vectors.linkpod[i];
														if(!j) break;
					...
					for(i = 0;i < 3;i++)
					{
						if((k + i) < CurrentIndex)
						{
							if(Route[k + i].state == GRS_ONTRAIN)
							{
								...
								{
									if(VectorCompare(Route[k + i].Vectors.Tcourner,/*e*/it_ent->target_ent->s.origin))
									{
										j = true;
										//it_ent = e;
										break;									
