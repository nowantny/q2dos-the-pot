...
//	gi.dprintf(DEVELOPER_MSG_GAME, "%s is not a field %s\n", key,ent->classname);
}
...
//		orgtargetname = e->targetname;
				...
				if(findteam && lc > 0)
				{
					gi.dprintf(DEVELOPER_MSG_GAME, "%i train chaining founded.\n",lc);
					for(k = 0;k < lc; k++)
					{
			...
			for(j = 0;j < CurrentIndex;j++)
			{
				...
				if(j != i && Route[i].ent == Route[j].ent)
				{
					for(k = 0;k < (MAXLINKPOD - (ctf->value != 0));k++)
					{
						...
						if(!Route[i].Vectors.linkpod[k])
						{
							Route[i].Vectors.linkpod[k] = j;
							break;
						}
					}
				}
		...
		if(Route[i].state == GRS_NORMAL)
		{
			for(j = 0;j < CurrentIndex;j++)
			{
				...
				for(l = -5;l < 6;l++)
				{
					...
					{
						//search blanked index
						if(!Route[i + l].Vectors.linkpod[k]) break;
						if(abs(Route[i + l].Vectors.linkpod[k] - j) < 50)
						{
							tpbool = true;
							break;
						}
...
//gi.dprintf(DEVELOPER_MSG_GAME, "yare!!!!!!!\n");
	if(ctf->value && bot_team_flag1 && bot_team_flag2)
	{
		...
		for(i = (CurrentIndex - 1);i >= 0 ;i--)
		{
			if(Route[i].state < GRS_ITEMS)
			{
				...
				if(j == FOR_FLAG1)		Route[i].Vectors.linkpod[MAXLINKPOD - 1] = (CTF_FLAG1_FLAG | k);
				else if(j == FOR_FLAG2)	Route[i].Vectors.linkpod[MAXLINKPOD - 1] = (CTF_FLAG2_FLAG | k);
				else Route[i].Vectors.linkpod[MAXLINKPOD - 1] = 0;
			}
		}
	}
