...
void Wav_Mod_Setup(void)
{
	...
	Com_sprintf(file_name, sizeof file_name, "%s/playlist.txt", gamedir->string);

	file = fopen(file_name, "r");

	if (!file)
	{
		gi.dprintf (DEVELOPER_MSG_GAME, "==== Wav Mod - missing playlist.txt file ====\n");
	...
	else
	{
		...
		gi.dprintf(DEVELOPER_MSG_GAME, "Allocated %ld bytes for file\n", file_size);
		gi.dprintf(DEVELOPER_MSG_GAME, "Lines in file: %d\n", lines);

		count = fread((void *)p_buffer, sizeof(char), file_size, file);
		if (count == 0 || ferror(file))
		{
			gi.dprintf (DEVELOPER_MSG_GAME, "Error reading %s\n", file_name);
			gi.dprintf (DEVELOPER_MSG_GAME, "Characters read: %d\n", count);
			gi.cvar_set ("use_song_file", "0");
		}

		gi.dprintf(DEVELOPER_MSG_GAME, "Adding Wav's to cycle: ");
		...
		do
		{
			...
			if (n_chars)
			{
				...
				if (wav_mod_num_wavs > 0)
					gi.dprintf(DEVELOPER_MSG_GAME, ", ");
				gi.dprintf(DEVELOPER_MSG_GAME, "%s", wav_mod_names[wav_mod_num_wavs]);
				...
				if (wav_mod_num_wavs >= MAXSONGS)
				{
					...
					gi.dprintf(DEVELOPER_MSG_GAME, "Unable to add more Wavs.\n");
					break;
				}
			}
...
void PrecacheSongs(void)
{
	...
	if (file == NULL)
	{
		gi.dprintf(DEVELOPER_MSG_GAME, "==== Wav Mod v.01 - missing playlist.txt file ====\n");
	}
	else
	{
		int file_size = 0;
		...
		count = fread((void *)p_buffer, sizeof(char), file_size, file);
		if (!count)
			gi.dprintf(DEVELOPER_MSG_GAME, "%s read %d of %d bytes in %s\n",
		...
		do
		{
			...
			if (n_chars)
			{
				...
				if (levels >= MAXSONGS)
				{
					gi.dprintf(DEVELOPER_MSG_GAME, "Wav Mod: MAXSONGS reached.\n");
					break;
				}
			}
