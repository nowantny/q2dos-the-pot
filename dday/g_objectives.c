...
//	if (!self->obj_time)
	...
	gi.dprintf(DEVELOPER_MSG_GAME, "distance: %f\n", 
		self->obj_area);

	gi.dprintf(DEVELOPER_MSG_GAME, "award: %i, loss: %i\n", 
		self->obj_gain,
		self->obj_loss);
	   
	gi.dprintf(DEVELOPER_MSG_GAME, "required persons: %i\n", self->obj_count);
	gi.dprintf(DEVELOPER_MSG_GAME, "must hold for %i seconds.\n\n",	(int)self->obj_time);
