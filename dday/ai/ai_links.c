...
int AI_GravityBoxStep( vec3_t origin, float scale, vec3_t destvec, vec3_t neworigin, vec3_t mins, vec3_t maxs )
{
	...
	if( trace.startsolid )
	{//gi.dprintf(DEVELOPER_MSG_GAME, "AI_GravityBoxStep startsolid  "); 
	return LINK_INVALID;}
	...
	if( gi.pointcontents( origin ) & MASK_WATER )
	{
		...
		{//gi.dprintf(DEVELOPER_MSG_GAME, "AI_GravityBoxStep origin = neworigin  "); 
		return LINK_INVALID;}
		...
		//gi.dprintf(DEVELOPER_MSG_GAME, "AI_GravityBoxStep  can't leave water  "); 
	...
	{	
		...
		//gi.dprintf(DEVELOPER_MSG_GAME, "AI_GravityBoxStep  can't step up \n");
		return LINK_INVALID;
	}
