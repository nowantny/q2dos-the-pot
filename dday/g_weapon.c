...
void fire_gun_old(edict_t *self, vec3_t start, vec3_t aimdir, int damage, int kick, int hspread, int vspread, int mod, qboolean calcv)
{
	...
	//gi.dprintf(DEVELOPER_MSG_GAME, "damage  %i\n", damage);
	//gi.dprintf(DEVELOPER_MSG_GAME, "kick    %i\n", kick);
	//gi.dprintf(DEVELOPER_MSG_GAME, "mod     %i\n", mod);
	//gi.dprintf(DEVELOPER_MSG_GAME, "calcv   %s\n", (calcv) ? "true":"false");

	// Useful debugging information
	//gi.dprintf(DEVELOPER_MSG_GAME, "hspread %i\n", hspread);
	//gi.dprintf(DEVELOPER_MSG_GAME, "vspread %i\n", vspread);
...
//		grenade->s.effects |= EF_GRENADE;
		...
		VectorClear (grenade->mins);
		VectorClear (grenade->maxs);
		//gi.dprintf(DEVELOPER_MSG_GAME, "grenade timer set to %f (%f)\n", time, level.time);
...
//	if(auto_weapon_change->value) NoAmmoWeaponChange (ent);
	...
	if (ent->client->pers.weapon->position == LOC_PISTOL)
		...
		gi.dprintf(DEVELOPER_MSG_GAME, "*** Firing System Error\n");
...
void TNT_RadiusDamage (edict_t *inflictor, edict_t *attacker, float damage, edict_t *ignore, float radius, int mod)
{
	...
	while ((ent = findradius(ent, inflictor->s.origin, radius)) != NULL)
	{
		...
		if (distance > radius/2)
			points = 0;
		//gi.dprintf(DEVELOPER_MSG_GAME, "%f %s\n", distance, ent->classname);
