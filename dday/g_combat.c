...
{	
	...
	//gi.dprintf(DEVELOPER_MSG_GAME, "maxs  (%f, %f, %f)\n", targ->maxs[0], targ->maxs[1], targ->maxs[2]);
	//gi.dprintf(DEVELOPER_MSG_GAME, "point (%f, %f, %f)\n", point[0], point[1], point[2]);
	//gi.dprintf(DEVELOPER_MSG_GAME, "Pmin  (%f, %f, %f)\n", min_x, min_y, min_z);
	//gi.dprintf(DEVELOPER_MSG_GAME, "Pmax  (%f, %f, %f)\n", max_x, max_y, max_z);
	...
	case STANCE_STAND:
		...
		{
			//gi.dprintf(DEVELOPER_MSG_GAME, "head\n");
			return HEAD_WOUND;
...
//faf		} else if (point[2] > min_z + 37 && point[2] < min_z + 46) {
			...
			//gi.dprintf(DEVELOPER_MSG_GAME, "chest\n");
			return CHEST_WOUND;
//faf		} else if (point[2] > min_z + 28 && point[2] < min_z + 37) {
			...
			//gi.dprintf(DEVELOPER_MSG_GAME, "stomach\n");
			return STOMACH_WOUND;
...
//faf		} else if (point[2] > min_z + 16 && point[2] < min_z + 26) {
			...
			//gi.dprintf(DEVELOPER_MSG_GAME, "stomache\n");
			return STOMACH_WOUND;
//faf		} else if (point[2] < min_z + 18) {
	...
	//gi.dprintf(DEVELOPER_MSG_GAME, "default wound\n");
	return STOMACH_WOUND;
	...
	//gi.dprintf(DEVELOPER_MSG_GAME, "h%f:w%f:d%f\n", height, width, depth);
	...
	{ //leg damage 
		...
		//gi.dprintf(DEVELOPER_MSG_GAME, "leg x%f:y%f:z%f\n", leg_x, leg_y, leg_z); 
		return LEG_WOUND;
	}
	...
	{ //stomach damage 
		...
		//gi.dprintf(DEVELOPER_MSG_GAME, "stom x%f:y%f:z%f\n", stom_x, stom_y, stom_z); 
		return STOMACH_WOUND;
	}
	...
	{ //chest damage 
		...
		//gi.dprintf(DEVELOPER_MSG_GAME, "chest x%f:y%f:z%f\n", chest_x, chest_y, chest_z); 
		return CHEST_WOUND;
	}
	...
	{ //head damage 
		...
		//gi.dprintf(DEVELOPER_MSG_GAME, "head x%f:y%f:z%f\n", head_x, head_y, head_z); 
		return HEAD_WOUND;
	}
		...
		if (check_ent->deadflag)
		 ...
		 if (visible (check_ent, targ))
		 {
			...
			if (sniper == false && inflictor->client->pers.weapon && inflictor->client->pers.weapon->position == LOC_SNIPER)
			{
				...
				//gi.dprintf(DEVELOPER_MSG_GAME, "SNIPER\n");
			}
		 }
...
// do the damage
	if (take)
	{
		...
		if (targ->health <= 0)
		{
			...
			//gi.dprintf(DEVELOPER_MSG_GAME, "attacker on i%\n",attacker->team_on->kills+5);
	...
	while ((ent = findradius(ent, inflictor->s.origin, radius)) != NULL)
	{
		...
		if (points > 0)
		{
			if (CanDamage (ent, inflictor))
			{
				...
				//gi.dprintf(DEVELOPER_MSG_GAME, "%f %i \n",  VectorLength (v), (int)points);
			}
		}
	}
...
//	int		j;
	...
	//gi.dprintf(DEVELOPER_MSG_GAME, "effect_radius: %f\n       radius: %f\n       damage: %f\n",
	//	effect_radius, radius, damage);
	...
	while ((dmgef_ent = findradius(dmgef_ent, inflictor->s.origin, effect_radius)) != NULL)
	{
		...
		//gi.dprintf(DEVELOPER_MSG_GAME, "VectorLength: %f (flash is < 275)\n", VectorLength (v));
