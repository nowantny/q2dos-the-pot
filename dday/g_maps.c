...
{ 
   ...
   { 
      // file did not load 
      gi.dprintf(DEVELOPER_MSG_GAME, "Could not open file \"%s\".\n", filename); 
      return NULL; 
   } 
...
{ 
   ...
   { 
      ...
      gi.dprintf(DEVELOPER_MSG_GAME, "ERROR -- DDay_CloseFile() exception.\n"); 
}
...
{ 
   ...
   { 
      ...
      { 
         ...
         gi.dprintf(DEVELOPER_MSG_GAME, "ERROR - No [maplist] section in \"%s\".\n", filename); 
         gi.dprintf(DEVELOPER_MSG_GAME, "-------------------------------------\n"); 
      } 
      else 
      { 
      ...
      if (i == 0) 
      { 
         gi.dprintf(DEVELOPER_MSG_GAME, "No maps listed in [maplist] section of %s\n", filename); 
         gi.dprintf(DEVELOPER_MSG_GAME, "-------------------------------------\n"); 
         return 0;  // abnormal exit -- no maps in file 
      } 
  
      gi.dprintf(DEVELOPER_MSG_GAME, "%i map(s) loaded.\n", i); 
      gi.dprintf(DEVELOPER_MSG_GAME, "-------------------------------------\n"); 
      maplist.nummaps = i; 
...
{ 
   ...
   gi.dprintf(DEVELOPER_MSG_GAME, "Maplist cleared/disabled.\n"); 
} 
...
{ 
   ...
   if (ent==NULL)     // only show filename to server 
      gi.dprintf(DEVELOPER_MSG_GAME, "FILENAME: %s\n", maplist.filename); 
   ...
   case 4:  // enable maplist - all args explicitly stated on command line 
      ...
      { 
         ...
         { 
            ...
            { 
               gi.dprintf(DEVELOPER_MSG_GAME, "*** Map# out of range ***\n"); 
      ...
      { 
         ...
         gi.dprintf(DEVELOPER_MSG_GAME, "You must disable current maplist first. (SV MAPLIST OFF)\n"); 
      } 
      else 
      { 
         // load new maplist 
         ...
         { 
            dmflags->value = (int) dmflags->value | DF_MAP_LIST; 
            gi.dprintf(DEVELOPER_MSG_GAME, "Maplist created/enabled. You can now use START or NEXT.\n"); 
