...
void Create_InfoTeamStarts (edict_t *self)
{
	...
	gi.dprintf(DEVELOPER_MSG_GAME, "WARNING: No info_team_start entities found.  Creating from scratch.\n");
...
/*	gi.WriteByte (svc_temp_entity);
	...
	gi.dprintf(DEVELOPER_MSG_GAME, "%i   %s",playercount,vtos(mean_origin));
*/
...
//faf			ent->client->pers.weapon->position == LOC_ROCKET	||
	...
	//gi.dprintf(DEVELOPER_MSG_GAME, "speednew: %s\n", vtos(speed_new));
	//gi.dprintf(DEVELOPER_MSG_GAME, "speedmax  %s\n", vtos(ent->client->speedmax));
}
...
qboolean Setup_Map_Vote (void)
{
	...
	{
		gi.dprintf(DEVELOPER_MSG_GAME, "Problem loading vote maps!\n");
		return false;
	}
...
// Nick 01/05/2003 - added && ent->stanceflags == STANCE_STAND to fix slope bug
 //       if (ent->client->aim && VectorLength(dist) > 25 && ent->velocity[2] != 0
   //            && ent->stanceflags == STANCE_STAND)
//		{
			//gi.dprintf(DEVELOPER_MSG_GAME, "kicked out: %f %s %s\n", VectorLength(dist), vtos(dist), vtos(ent->velocity));
	//		Cmd_Scope_f(ent);
///		}

		// jump cheat fix
/*
		gi.dprintf(DEVELOPER_MSG_GAME, "1 %3i (up %3i) jump_stuck: %5s jump_last: %5s %s\n",
...
*/
		...
		if (client->jump_stuck && !client->jump_last)
			ucmd->upmove = 0;
/*
		gi.dprintf(DEVELOPER_MSG_GAME, "2 %3i (up %3i) jump_stuck: %5s jump_last: %5s %s\n",
			(int)client->jump_stamina , ucmd->upmove,
			(client->jump_stuck) ? "true" : "false",
			(client->jump_last)  ? "true" : "false",
			vtos(ent->velocity) );
...
//		ClientSetMaxSpeed(ent, true);
	...
	{
		//gi.dprintf(DEVELOPER_MSG_GAME, "groundentity: %s\n", (ent->groundentity) ? "true" : "false");
...
//		gi.dprintf(DEVELOPER_MSG_GAME, "pmove changed!\n");
	...
	if (!ent->deadflag)
	{
		...
		//gi.dprintf(DEVELOPER_MSG_GAME, "maxs  %f\n", pm.maxs[2]);
		//gi.dprintf(DEVELOPER_MSG_GAME, "maxs  %s\n", vtos (pm.maxs));
		//gi.dprintf(DEVELOPER_MSG_GAME, "mins  %s\n", vtos (pm.mins));
	}
	//END DDAY
	//
