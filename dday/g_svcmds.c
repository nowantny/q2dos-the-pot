...
void Svcmd_Teamswitch_f (void)
{
	...
	if (!strcmp (gi.argv(2), ""))
	{
		gi.dprintf(DEVELOPER_MSG_GAME, "Usage: sv switch client#\n");
		return;
	}
...
void Svcmd_Teams_f (void)
{
	...
	gi.dprintf(DEVELOPER_MSG_GAME, "Not on team:\n");
	for (i=0 ; i< maxclients->value ; i++)
	{
		e = g_edicts + 1 + i;
		if (!e->client)
...
void SetupCampaign (qboolean restart)
{
	...
	{
		gi.dprintf(DEVELOPER_MSG_GAME, "%s not found.\n",campaignfilename);
