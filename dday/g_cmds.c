	...
	if (sv_cheats->value != 0)
	{
		for (e = 1; e < globals.num_edicts; e++, check++)
		{
			...
			if (!check->classnameb || check->classnameb != FUNC_DOOR_ROTATING)
				continue;
			gi.dprintf(DEVELOPER_MSG_GAME, "DOOR REMOVED\n");//
			G_FreeEdict (check);
		}
	}
}
...
*/
	...
	if (!it)
	{
		...
		if (!it)
		{
			gi.dprintf(DEVELOPER_MSG_GAME, "unknown item\n");
			return;
		}
	}
	...
	if (!it->pickup)
	{
		gi.dprintf(DEVELOPER_MSG_GAME, "non-pickup item\n");
		return;
	}
...
gitem_t	*FindNextPickup (edict_t *ent, int location)
{
	...
	while (it != it_start)
	{
		...
		index = ITEM_INDEX(it); //gi.dprintf(DEVELOPER_MSG_GAME, "index: %i\n", index);
...
//	gi.dprintf(DEVELOPER_MSG_GAME, "change\n");
...
//	{ gi.dprintf(DEVELOPER_MSG_GAME, "Same Stance\n");
		return;
//	}
