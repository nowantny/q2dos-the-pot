...
edict_t *find_targetname(char *targetname)
{
   ...
   gi.dprintf(DEVELOPER_MSG_GAME, "name <%s> not found\n", targetname);
   return NULL;
}
...
{
   gi.dprintf(DEVELOPER_MSG_GAME, "%s %s ", targetname, description);
   if(on)
      gi.dprintf(DEVELOPER_MSG_GAME, "ON\n");
   else
      gi.dprintf(DEVELOPER_MSG_GAME, "OFF\n");
}
...
qboolean anim_player_capture(char *targetname)
{
   ...
   for(i=0;i<animations_count;i++)
   {
      ...
      {
         gi.dprintf(DEVELOPER_MSG_GAME, "Target <%s> already captured\n", targetname);
         return false;
      }
   }
   ...
   {
      gi.dprintf(DEVELOPER_MSG_GAME, "Maximum of %d animations already used\n", MAX_ANIMATIONS);
      return false;
   }
   ...
   if(anim)
   {
      ...
      gi.dprintf(DEVELOPER_MSG_GAME, "Target <%s> captured\n", targetname);
   }
   else
      gi.dprintf(DEVELOPER_MSG_GAME, "Target <%s> NOT captured\n", targetname);
...
void anim_player_cmd(edict_t *ent)
{
   ...
   if(Q_stricmp (arg1, "capture") == 0)
      ...
      gi.dprintf(DEVELOPER_MSG_GAME, "unknown anim command <%s>\n", arg1);
