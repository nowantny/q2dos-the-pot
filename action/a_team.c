...
void RemoveFromTransparentList(edict_t *ent)
{
        ...
        gi.dprintf(DEVELOPER_MSG_GAME, "Warning: attempt to RemoveFromTransparentList when not in it\n");
}               
...
void GetSpawnPoints()
{
        ...
        while ((spot = G_Find (spot, FOFS(classname), "info_player_deathmatch")) != NULL)
        {
                ...
                {
                        gi.dprintf(DEVELOPER_MSG_GAME, "Warning: MAX_SPAWNS exceeded\n");
                        return;
                }
        }
}
...
//FB 6/1/99
	...
	{
		gi.dprintf(DEVELOPER_MSG_GAME, "Out-of-range teams value in SelectFarTeamplaySpawnPoint, skipping...\n");
	}
		else
	{
	        teams_assigned[team] = true;
...
void SetupTeamSpawnPoints()
{
        ...
        do
        {
                if (l < 0 || l >= MAX_TEAMS)
                {
                        gi.dprintf(DEVELOPER_MSG_GAME, "Warning: attempt to setup spawns for out-of-range team (%d)\n", l);
                }
