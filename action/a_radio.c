...
void DeleteFirstRadioQueueEntry(edict_t *ent)
{
        ...
        {
                gi.dprintf(DEVELOPER_MSG_GAME, "DeleteFirstRadioQueueEntry: attempt to delete without any entries\n");
                return;
        }
...
void DeleteRadioQueueEntry(edict_t *ent, int entry_num)
{
        ...
        {
                gi.dprintf(DEVELOPER_MSG_GAME, "DeleteRadioQueueEntry: attempt to delete out of range queue entry\n");
                return;
        }
...
void AppendRadioMsgToQueue(edict_t *ent, char *msg, int len, int click, edict_t *from_player)
{
        ...
        {
                gi.dprintf(DEVELOPER_MSG_GAME, "AppendRadioMsgToQueue: Maximum radio queue size exceeded\n");
                return;
        }
        ...
        {
                gi.dprintf(DEVELOPER_MSG_GAME, "Radio sound file path (%s) exceeded maximum length\n", msg);
...
void InsertRadioMsgInQueueBeforeClick(edict_t *ent, char *msg, int len, edict_t *from_player)
{
        ...
        if (ent->client->resp.radio_queue_size >= MAX_RADIO_QUEUE_SIZE)
        {
                gi.dprintf(DEVELOPER_MSG_GAME, "InsertRadioMsgInQueueBeforeClick: Maximum radio queue size exceeded\n");
                return;
        }
