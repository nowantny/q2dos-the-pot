...
//FIREBLADE
        if (!deathmatch->value)
        {
                gi.dprintf(DEVELOPER_MSG_GAME, "Turning deathmatch on.\n");
                gi.cvar_set("deathmatch", "1");
        }
        if (coop->value)
        {
                gi.dprintf(DEVELOPER_MSG_GAME, "Turning coop off.\n");
                gi.cvar_set("coop", "0");
        }
//FIREBLADE
...
//FIREBLADE
	needpass = gi.cvar("needpass", "0", CVAR_SERVERINFO);
        ...
        {
                gi.dprintf(DEVELOPER_MSG_GAME, "No maps were read from the config file, \"actionmaps\" won't be used.\n");
