...
//FIREBLADE
                if (deathmatch->value)
                {
                        gi.dprintf(DEVELOPER_MSG_GAME, "Warning: failed to find deathmatch spawn point\n");
                }
//FIREBLADE
...
//FIREBLADE
	if (strlen(ipaddr) > sizeof(ipaddr_buf)-1)
		gi.dprintf(DEVELOPER_MSG_GAME, "ipaddr_buf length exceeded\n");
...
*/
        ...
        if (game.maxclients > 1)
                gi.dprintf(DEVELOPER_MSG_GAME, "%s@%s connected\n",ent->client->pers.netname, ipaddr_buf);
//EEK
